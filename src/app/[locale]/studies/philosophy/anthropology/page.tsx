import { Metadata } from "next"
import { Shell } from "@/components/shells/shell"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import Link from "next/link"

export const metadata: Metadata = {
  title: "Religious Anthropology & Sacred Culture | Sacred Studies",
  description: "Explore religious anthropology and sacred cultural studies across traditions. In-depth analysis of cultural methods, religious practices, and contemporary approaches.",
  openGraph: {
    title: "Religious Anthropology & Sacred Culture",
    description: "Comprehensive exploration of religious anthropology and cultural traditions.",
    type: "website",
    images: [{ url: "/images/og/religious-anthropology.jpg" }]
  },
  keywords: [
    "religious anthropology",
    "sacred culture",
    "cultural studies",
    "religious practices",
    "anthropological methods",
    "sacred traditions",
    "cultural analysis",
    "religious rituals",
    "anthropological research",
    "sacred society"
  ]
}

interface AnthropologicalTradition {
  tradition: string
  overview: string
  methods: {
    name: string
    description: string
    principles: string[]
    techniques: string[]
    applications: string[]
  }[]
  resources: {
    title: string
    link: string
    description: string
  }[]
}

const anthropologicalTraditions: AnthropologicalTradition[] = [
  {
    tradition: "Sacred Culture",
    overview: "Traditional approaches to studying religious cultures",
    methods: [
      {
        name: "Cultural Analysis",
        description: "Systematic study of religious cultural practices",
        principles: [
          "Cultural context",
          "Sacred traditions",
          "Religious practices",
          "Social dynamics"
        ],
        techniques: [
          "Ethnographic study",
          "Cultural observation",
          "Practice analysis",
          "Social research"
        ],
        applications: [
          "Cultural understanding",
          "Religious insight",
          "Social knowledge",
          "Teaching methods"
        ]
      },
      {
        name: "Ritual Studies",
        description: "Investigation of religious rituals and ceremonies",
        principles: [
          "Ritual meaning",
          "Sacred practice",
          "Symbolic action",
          "Cultural significance"
        ],
        techniques: [
          "Ritual observation",
          "Symbolic analysis",
          "Practice study",
          "Cultural interpretation"
        ],
        applications: [
          "Ritual understanding",
          "Cultural teaching",
          "Religious practice",
          "Social insight"
        ]
      }
    ],
    resources: [
      {
        title: "Sacred Culture: A Guide",
        link: "https://www.amazon.com/Sacred-Culture-Guide/dp/0195114523?tag=zion0ba-20",
        description: "Comprehensive guide to religious cultural studies"
      }
    ]
  }
]

interface ResearchMethod {
  method: string
  description: string
  aspects: {
    name: string
    principles: string[]
    practices: string[]
    outcomes: string[]
  }[]
  resources: {
    title: string
    link: string
    description: string
  }[]
}

const researchMethods: ResearchMethod[] = [
  {
    method: "Anthropological Research",
    description: "Methods for studying religious cultures and practices",
    aspects: [
      {
        name: "Field Research",
        principles: [
          "Direct observation",
          "Cultural immersion",
          "Participant study",
          "Ethical research"
        ],
        practices: [
          "Field notes",
          "Cultural mapping",
          "Practice documentation",
          "Interview methods"
        ],
        outcomes: [
          "Cultural insight",
          "Religious understanding",
          "Social knowledge",
          "Teaching resources"
        ]
      },
      {
        name: "Analytical Methods",
        principles: [
          "Cultural analysis",
          "Comparative study",
          "Systematic research",
          "Theoretical framework"
        ],
        practices: [
          "Data analysis",
          "Cultural comparison",
          "Theory development",
          "Research synthesis"
        ],
        outcomes: [
          "Theoretical insight",
          "Cultural knowledge",
          "Research findings",
          "Teaching materials"
        ]
      }
    ],
    resources: [
      {
        title: "Anthropological Methods",
        link: "https://www.amazon.com/Anthropological-Methods/dp/0861711734?tag=zion0ba-20",
        description: "Guide to anthropological research methods"
      }
    ]
  }
]

interface ContemporaryApproach {
  approach: string
  description: string
  elements: {
    name: string
    features: string[]
    methods: string[]
    implications: string[]
  }[]
  resources: {
    title: string
    link: string
    description: string
  }[]
}

const contemporaryApproaches: ContemporaryApproach[] = [
  {
    approach: "Modern Anthropology",
    description: "Contemporary approaches to religious cultural studies",
    elements: [
      {
        name: "Digital Methods",
        features: [
          "Digital tools",
          "Online research",
          "Virtual ethnography",
          "Data analysis"
        ],
        methods: [
          "Digital observation",
          "Online fieldwork",
          "Virtual interviews",
          "Data collection"
        ],
        implications: [
          "Broader reach",
          "New insights",
          "Global perspective",
          "Research innovation"
        ]
      },
      {
        name: "Integrative Approaches",
        features: [
          "Multiple methods",
          "Cross-cultural study",
          "Interdisciplinary work",
          "Global perspective"
        ],
        methods: [
          "Mixed methods",
          "Comparative analysis",
          "Cultural synthesis",
          "Global research"
        ],
        implications: [
          "Rich understanding",
          "Cultural insight",
          "Global knowledge",
          "Research impact"
        ]
      }
    ],
    resources: [
      {
        title: "Modern Religious Anthropology",
        link: "https://www.amazon.com/Modern-Religious-Anthropology/dp/0865714501?tag=zion0ba-20",
        description: "Guide to modern approaches in religious anthropology"
      }
    ]
  }
]

export default function ReligiousAnthropologyPage() {
  return (
    <Shell>
      <div className="container mx-auto px-4 py-8 space-y-12">
        <section className="space-y-4">
          <h1 className="text-4xl font-bold">Religious Anthropology & Sacred Culture</h1>
          <p className="text-xl text-muted-foreground">
            Explore anthropological traditions and methods of studying religious cultures
          </p>
        </section>

        <Tabs defaultValue="traditions" className="space-y-8">
          <TabsList className="grid w-full grid-cols-1 md:grid-cols-3">
            <TabsTrigger value="traditions">Anthropological Traditions</TabsTrigger>
            <TabsTrigger value="methods">Research Methods</TabsTrigger>
            <TabsTrigger value="contemporary">Contemporary Approaches</TabsTrigger>
          </TabsList>

          <TabsContent value="traditions" className="space-y-8">
            <div className="grid grid-cols-1 gap-6">
              {anthropologicalTraditions.map((tradition) => (
                <Card key={tradition.tradition}>
                  <CardHeader>
                    <CardTitle>{tradition.tradition}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-6">
                      <p className="text-muted-foreground">{tradition.overview}</p>

                      {tradition.methods.map((method) => (
                        <div key={method.name} className="space-y-4">
                          <h3 className="text-xl font-semibold">{method.name}</h3>
                          <p className="text-muted-foreground">{method.description}</p>

                          <div className="grid gap-4 md:grid-cols-2">
                            <div className="space-y-2">
                              <h4 className="font-semibold">Principles</h4>
                              <ul className="list-disc list-inside text-sm text-muted-foreground">
                                {method.principles.map((principle) => (
                                  <li key={principle}>{principle}</li>
                                ))}
                              </ul>
                            </div>

                            <div className="space-y-2">
                              <h4 className="font-semibold">Techniques</h4>
                              <ul className="list-disc list-inside text-sm text-muted-foreground">
                                {method.techniques.map((technique) => (
                                  <li key={technique}>{technique}</li>
                                ))}
                              </ul>
                            </div>
                          </div>

                          <div className="space-y-2">
                            <h4 className="font-semibold">Applications</h4>
                            <ul className="list-disc list-inside text-sm text-muted-foreground">
                              {method.applications.map((application) => (
                                <li key={application}>{application}</li>
                              ))}
                            </ul>
                          </div>
                        </div>
                      ))}

                      <div className="space-y-2">
                        <h4 className="font-semibold">Resources:</h4>
                        {tradition.resources.map((resource) => (
                          <div key={resource.title} className="space-y-1">
                            <a
                              href={resource.link}
                              className="block w-full text-center bg-primary text-primary-foreground px-4 py-2 rounded-md hover:bg-primary/90"
                              target="_blank"
                              rel="noopener noreferrer"
                            >
                              {resource.title}
                            </a>
                            <p className="text-sm text-muted-foreground text-center">{resource.description}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ))}
            </div>
          </TabsContent>

          <TabsContent value="methods" className="space-y-8">
            <div className="grid grid-cols-1 gap-6">
              {researchMethods.map((method) => (
                <Card key={method.method}>
                  <CardHeader>
                    <CardTitle>{method.method}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-6">
                      <p className="text-muted-foreground">{method.description}</p>

                      {method.aspects.map((aspect) => (
                        <div key={aspect.name} className="space-y-4">
                          <h3 className="text-xl font-semibold">{aspect.name}</h3>

                          <div className="grid gap-4 md:grid-cols-2">
                            <div className="space-y-2">
                              <h4 className="font-semibold">Principles</h4>
                              <ul className="list-disc list-inside text-sm text-muted-foreground">
                                {aspect.principles.map((principle) => (
                                  <li key={principle}>{principle}</li>
                                ))}
                              </ul>
                            </div>

                            <div className="space-y-2">
                              <h4 className="font-semibold">Practices</h4>
                              <ul className="list-disc list-inside text-sm text-muted-foreground">
                                {aspect.practices.map((practice) => (
                                  <li key={practice}>{practice}</li>
                                ))}
                              </ul>
                            </div>
                          </div>

                          <div className="space-y-2">
                            <h4 className="font-semibold">Outcomes</h4>
                            <ul className="list-disc list-inside text-sm text-muted-foreground">
                              {aspect.outcomes.map((outcome) => (
                                <li key={outcome}>{outcome}</li>
                              ))}
                            </ul>
                          </div>
                        </div>
                      ))}

                      <div className="space-y-2">
                        <h4 className="font-semibold">Resources:</h4>
                        {method.resources.map((resource) => (
                          <div key={resource.title} className="space-y-1">
                            <a
                              href={resource.link}
                              className="block w-full text-center bg-primary text-primary-foreground px-4 py-2 rounded-md hover:bg-primary/90"
                              target="_blank"
                              rel="noopener noreferrer"
                            >
                              {resource.title}
                            </a>
                            <p className="text-sm text-muted-foreground text-center">{resource.description}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ))}
            </div>
          </TabsContent>

          <TabsContent value="contemporary" className="space-y-8">
            <div className="grid grid-cols-1 gap-6">
              {contemporaryApproaches.map((approach) => (
                <Card key={approach.approach}>
                  <CardHeader>
                    <CardTitle>{approach.approach}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-6">
                      <p className="text-muted-foreground">{approach.description}</p>

                      {approach.elements.map((element) => (
                        <div key={element.name} className="space-y-4">
                          <h3 className="text-xl font-semibold">{element.name}</h3>

                          <div className="grid gap-4 md:grid-cols-2">
                            <div className="space-y-2">
                              <h4 className="font-semibold">Features</h4>
                              <ul className="list-disc list-inside text-sm text-muted-foreground">
                                {element.features.map((feature) => (
                                  <li key={feature}>{feature}</li>
                                ))}
                              </ul>
                            </div>

                            <div className="space-y-2">
                              <h4 className="font-semibold">Methods</h4>
                              <ul className="list-disc list-inside text-sm text-muted-foreground">
                                {element.methods.map((method) => (
                                  <li key={method}>{method}</li>
                                ))}
                              </ul>
                            </div>
                          </div>

                          <div className="space-y-2">
                            <h4 className="font-semibold">Implications</h4>
                            <ul className="list-disc list-inside text-sm text-muted-foreground">
                              {element.implications.map((implication) => (
                                <li key={implication}>{implication}</li>
                              ))}
                            </ul>
                          </div>
                        </div>
                      ))}

                      <div className="space-y-2">
                        <h4 className="font-semibold">Resources:</h4>
                        {approach.resources.map((resource) => (
                          <div key={resource.title} className="space-y-1">
                            <a
                              href={resource.link}
                              className="block w-full text-center bg-primary text-primary-foreground px-4 py-2 rounded-md hover:bg-primary/90"
                              target="_blank"
                              rel="noopener noreferrer"
                            >
                              {resource.title}
                            </a>
                            <p className="text-sm text-muted-foreground text-center">{resource.description}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ))}
            </div>
          </TabsContent>
        </Tabs>

        <section className="space-y-6">
          <h2 className="text-3xl font-semibold">Related Resources</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Link 
              href="/studies/philosophy/sociology"
              className="block p-4 bg-secondary rounded-lg hover:bg-secondary/90"
            >
              Religious Sociology →
            </Link>
            <Link 
              href="/studies/philosophy/psychology"
              className="block p-4 bg-secondary rounded-lg hover:bg-secondary/90"
            >
              Religious Psychology →
            </Link>
            <Link 
              href="/studies/themes/culture/traditions"
              className="block p-4 bg-secondary rounded-lg hover:bg-secondary/90"
            >
              Sacred Traditions →
            </Link>
            <Link 
              href="/studies/themes/society/communities"
              className="block p-4 bg-secondary rounded-lg hover:bg-secondary/90"
            >
              Religious Communities →
            </Link>
          </div>
        </section>
      </div>
    </Shell>
  )
} 