...
import { ChevronLeft, ChevronRight, BookOpen, Search, BookMarked, Users, Calendar, ScrollText } from "lucide-react"
...
export default async function TorahPortionPage({ params }: TorahPortionPageProps) {
  const { portion, locale } = params
  const portionData = await torahService.getPortion(portion)
  if (!portionData) notFound()

  // Get all chapters for this portion
  const chapters = portionData.chapters
  const allVerses: { [chapter: string]: any[] } = {}

  for (const chapter of chapters) {
    try {
      const verses = await torahService.getChapter(portion, chapter, 'ENGLISH')
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-4 mb-4">
          <Button variant="outline" asChild size="sm">
            <Link href={`/${locale}/sacred-texts/torah/${portionData.book.toLowerCase()}`}>
              <ChevronLeft className="h-4 w-4 mr-2" />
              Back to {portionData.book}
            </Link>
          </Button>
          <Button variant="outline" asChild size="sm">
            <Link href={`/${locale}/sacred-texts/torah`}>
        ...
        <div className="text-center">
          <h1 className="text-4xl font-bold mb-2">{portionData.hebrewName} - {portionData.name}</h1>
          <h2 className="text-2xl text-muted-foreground mb-4">{portionData.transliteratedName}</h2>
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto mb-6">
            {portionData.summary}
          </p>
          <div className="flex flex-wrap gap-2 justify-center mb-6">
            {portionData.themes.map((theme) => (
              <Badge key={theme} variant="secondary">{theme}</Badge>
            ))}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-2xl mx-auto">
            <div className="text-center">
              <div className="text-2xl font-bold">{portionData.verses}</div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold">{portionData.book}</div>
      ...
      {portionData.haftarah && (
        <Card className="mb-8">
          ...
          <CardContent>
            <p className="text-lg mb-2">{portionData.haftarah}</p>
        ...
        {chapters.map((chapter) => (
          <Card key={chapter} className="mb-6">
            <CardHeader>
              ...
              <CardDescription>
                {portionData.name} {chapter} - {allVerses[chapter]?.length || 0} verses
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <Button asChild size="sm">
                <Link href={`/${locale}/sacred-texts/torah`}>Browse All</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          {/* Next/Prev navigation could be added here if desired */}
        </div>
      </div>
    </div>
  )
